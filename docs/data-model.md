...
- Tauri commands: `create_note`, `get_or_create_daily_note`, `get_note`,
  ...
  (`src-tauri/src/calendar.rs`); `create_meeting`, `get_meeting`,
  `update_meeting` (`src-tauri/src/meetings.rs`). `list_calendar_events` takes optional `from` /
...
- Frontend: `src/notes/` wraps note/link commands (`notesApi` + `useNotes` with
  ...
  `src/meetings/` wraps meeting metadata (`meetingsApi`) for the compact header.
